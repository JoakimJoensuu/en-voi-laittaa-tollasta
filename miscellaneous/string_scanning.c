...
void append_char(char_list target, char appendable) {
    if (target->size - target->length * sizeof(char) < sizeof(char)) {
...
void append_n_char(char_list target, const char* src, int count) {
    if (count == 0) {
        return;
    }
    ...
    memcpy(target->data + target->length, src, count * sizeof(char));
...
void append_string(char_list target, char* string) {
    ...
    memcpy(&target->data[target->length], string, length * sizeof(char));

    target->length += length;
}

int scan_string(const char* start_of_a_string, char** destination) {
    char_list result = init_char_list();

    for (const char *start = start_of_a_string, *current = start_of_a_string;;
         current++) {
...
char* find_string(char* text) {
    char* start_of_a_string = find_beginning_of_a_string(text);
...
int main() {
    ...
    for (char** text = texts; text - texts < sizeof(texts) / sizeof(*texts);
         text++) {
        printf("TEXT:\n%s\n\n", *text);
